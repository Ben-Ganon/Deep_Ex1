...
def softmax(x):
    ...
    """
    # For numeric stability, use to identify you proved in Ex 2 Q1 - softmax(x) = softmax(x+c)
    c = np.max(x)
    x -= c
    exp_x = np.exp(x)
...
def loss_and_gradients(x, y, params):
    ...
    """
    W,b = params
    probs = classifier_output(x, params)  # Get the probabilities of each class
    loss = -np.log(probs[y]) # Get the loss
    probs[y] -= 1 # Get the gradients
    gW = np.outer(x, probs) # Get the gradients of W
    gb = probs # Get the gradients of b
    return loss,[gW,gb]
